...
// Output calls atmos terraform output for the given variable and return its string value representation.
...
// OutputE calls atmos terraform output for the given variable and return its string value representation.
// It only designed to work with primitive terraform types: string, number and bool.
...
func OutputJsonE(t testing.TestingT, options *Options, key string) (string, error) {
	args := []string{"terraform", "output", options.Component, "--skip-init", "-json", "-s", options.Stack}
	if key != "" {
